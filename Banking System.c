...
int main(void)
{
	...
	if(temp==7)
	{
		...
		fprintf(fp,"Monthy Income = %d\nRequired amount of loan = %.2f\nLoan Repayment time(Years) = %.2f\nRepay = %.2f",s1.income,s1.loan_amount,s1.loan_repay_time,s1.repay);
		fclose(fp);
	}
	if(temp==0)
	{
